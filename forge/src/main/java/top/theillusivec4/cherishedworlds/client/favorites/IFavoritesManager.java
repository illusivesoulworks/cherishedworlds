...
public interface IFavoritesManager<T extends Screen> {
  ...
  void draw(ScreenEvent.DrawScreenEvent.Post evt, T screen);

  void click(ScreenEvent.MouseClickedEvent.Pre evt, T screen);

  void clicked(T screen);

  int getOffset();

  default void drawIcon(ScreenEvent.DrawScreenEvent.Post evt, Screen gui, int index,
    ...
    if (top < (bottom - 8) && top > topOffset) {
      ...
      GuiComponent.blit(evt.getPoseStack(), x, top, 0, 0, 9, 9, 9, 9);
    }
    ...
    if (mouseY >= top && mouseY <= (top + 9) && mouseX >= x && mouseX <= (x + 9)) {
      TranslatableComponent component = new TranslatableComponent(
          "selectWorld." + CherishedWorldsMod.MOD_ID + "." + (isFavorite ? "unfavorite"
              : "favorite"));
      gui.renderTooltip(evt.getPoseStack(), component, mouseX, mouseY);
    }
  }
}
